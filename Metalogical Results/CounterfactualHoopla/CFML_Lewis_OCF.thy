...
abbreviation mdiamond :: "\<sigma> \<Rightarrow> \<sigma>" ("\<^bold>\<diamond>") where "\<^bold>\<diamond> \<phi> \<equiv> \<^bold>\<not> (\<^bold>\<box> (\<^bold>\<not>\<phi>))"
...
lemma L5: "\<lfloor>((\<psi> \<^bold>\<and> \<phi>)  \<^bold>\<rightarrow> (\<phi> \<box>\<rightarrow>  \<psi>)) \<rfloor> "
  using LVC Cent by metis

lemma RCK: "\<lfloor>list_conj \<chi>s  \<^bold>\<rightarrow> \<psi>\<rfloor> \<Longrightarrow> \<lfloor>list_box_conj \<chi>s \<phi>  \<^bold>\<rightarrow> (\<phi> \<box>\<rightarrow> \<psi>)\<rfloor>"
  using LVC Cent totale Preorder by blast
