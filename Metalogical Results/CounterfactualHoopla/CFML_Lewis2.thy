...
abbreviation bottom :: "\<sigma>" ("\<bottom>") 
  where "\<bottom> (w) \<equiv> w \<^bold>\<Turnstile> (\<^bold>\<exists>x. ( \<^bold>\<not> (x \<^bold>=\<^sup>L x)  \<^bold>\<and>  (x \<^bold>=\<^sup>L x) ))"
abbreviation mbox :: "\<sigma> \<Rightarrow> \<sigma>" ("\<^bold>\<box>") where "\<^bold>\<box> \<phi> w \<equiv> ( \<not>( \<exists>v\<in>Ww(w). (\<^bold>\<not> \<phi>) v))" (*alterantive phi box\<rightarrow> bottom *)
...
abbreviation euclidean :: "i \<Rightarrow> bool"
  where "euclidean \<equiv> (\<lambda>w. \<forall>x y z. ((r w (x, y) \<and> r w (x, z)) \<longrightarrow> (r w (y, z))))"    
...
axiomatization where 
Cent: "\<forall>w. {w} = Lew_Minset w (\<^bold>\<not> \<bottom>)"
and TautBot: " \<forall>w. (w  \<^bold>\<Turnstile> (\<^bold>\<not> \<bottom>))"
...
lemma L3: "\<lfloor>(\<phi> \<box>\<rightarrow> (\<^bold>\<not>\<psi>))  \<^bold>\<or> (((\<psi> \<^bold>\<and> \<phi>) \<box>\<rightarrow> \<zeta>)  \<^bold>\<leftrightarrow> (\<phi> \<box>\<rightarrow>  (\<psi> \<^bold>\<rightarrow> \<zeta>) )) \<rfloor>"
  sledgehammer[timeout = 500]
   (* using Cent LVC totale Preorder Lew_Minset_def domr r_Ww_coherence by blast *)
